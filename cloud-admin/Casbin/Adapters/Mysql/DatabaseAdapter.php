...
class DatabaseAdapter implements Adapter, BatchAdapter, UpdatableAdapter, FilteredAdapter
{
    ...
    public function __construct(ContainerInterface $container, string $tableName)
    {
        ...
        $this->eloquent = \Hyperf\Support\make(
            Rule::class,
            ['attributes' => [], 'table' => $this->tableName],
        );
    ...
    public function savePolicy(Model $model): void
    {
        ...
        $this
            ->eventDispatcher
            ->dispatch(new PolicyChanged(__METHOD__, func_get_args()));
    }
    ...
    public function removeFilteredPolicy(
        ...
        foreach (range(0, 5) as $value) {
            if (
                $fieldIndex <= $value
                && $value < $fieldIndex + count($fieldValues)
